...
1. `Boost` 开关：单入口启停优化。
2. 视口优先单规则引擎：视口外优先降级，视口附近按需恢复。
3. 渲染三态：`full / collapsed / placeholder`。
4. 脱水与恢复：远距消息可脱水，按需恢复。
5. 内存护栏：会话级预算 + LRU 回退。
6. 问题优先索引：点击跳转 + 上下导航。
7. 最新消息保护：降低误折叠概率。
8. SPA 会话切换重建：路由变化自动 reset/rebuild。
...
1. 极长会话应急模式（Panic mode）。
2. 自动化基准与回归报告。
3. Claude / Gemini 适配对齐。
4. 宿主 DOM 变更下的稳定性加固。
